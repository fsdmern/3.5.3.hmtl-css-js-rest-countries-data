...
searchCountry.addEventListener('input', (e) => {
  ...
  const availableCountries = Array.from(
    document.querySelectorAll('.country-details-title')
  );
...
continentSelect.onchange = (evt) => {
  const availableCountries = Array.from(
    document.querySelectorAll('.country-region span')
  );
  availableCountries.forEach((country) => {
    const myCountry = country.innerHTML.toLowerCase().trim();
...
const modalTemplate = (element) => {
  ...
  borderingCountries.addEventListener('click', (evt) => {
    ...
    fetch(apiEndpoint)
      .then((response) => response.json())
      .then((data) => {
        //call the modalTemplate and fetched data for a particular country
        borderArray = [];
